...
class MDX_PARSER{
    ...
    async Parse(cwd,callback){
        return  readFile(cwd,"utf-8",(err,data)=>{
            if(data==undefined||err) return callback("<h1>there is no file here</h1>",null);
    ...
    mdx_block_parser(block){
        ...
        transforms_data = transforms_data.map((obj,i)=>{
            ...
            //handle links inside code section - need to be extracted
            var re = /(\[.*?\])/g
            var re2 = /(\(.*?\))/g
            const html_links =(ref.match(/\<a.*\>/gm)||[]); 
            const html_transform_box = html_links.map(x=>({o:x,t:x}))
            const md_links = (ref.match(/[^!](\[(.*?)\]\(.*?\))/g))||[]
            const md_html = md_links.map(link=>{
                const m1 = link.match(re2)[0].replace("(","").replace(")","");
                const m2 = link.match(re)[0].replace("]","").replace("[","");
            ...
            fake_links.forEach((fl,i)=>obj.t=obj.t.replaceAll(fl,links[i].t))
    ...
    scp(txt,internal_data_flag){
        ...
        switch(txt[0]){
            ...
            case "\~": return `<span class="description">${this.trim_deliminator(txt)}</span>`;
            case "\*": return `<span class="action">${this.trim_deliminator(txt)}</span>`;
            case "\`": return `<code class="code";>${this.trim_deliminator(txt)}</code>`;
        }
    }
